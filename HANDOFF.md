...
- **结果**：`node tests/verify-full.mjs` → **ALL PASS (clean exit)**（build / unit 469 / chat:e2e / preview:e2e 18/18），证据已更新至 §2.7 与 tests/V0\_4\_GATES.md。

- **注意**：工作目录为 `c:\Users\C\DoubaoWork\chats\2026-08-31\new-chat-1\dave-desktop`（非上面旧路径，脚本为历史遗留）

### 1l. 审查修复收尾（M5/H3b/Low，2026-09-01）

- M5：chat-stream-state patch 从"替换 content"改为"独立载体不污染正文"（diff 不进入正文流，后续 chunk 正常累加）；同步更新 chat-stream-state 用例 12 与 event-contract（e）黄金断言「patch 不覆盖 content，正文保持并继续累加」

- H3b/M3：ipc-push-guard 新增用例 10「豁免面契约」——error/tools/approval/patch 任意业务状态推送不触发守卫（固化既有豁免设计，防未来误配 mapEvent）

- L1：push-channels 注释 6→7 通道；L2：aborted 标签与 Sidebar ready 状态 i18n 化（新增 common.ready）

- E2E H1 断言改轮询（消除 updateTitle 落库时序竞态）

验证实测：typecheck 0；vitest **462/462**；build 绿（renderer 1112KB）；真实会话 E2E 3 断言全过、零 console 错误。
目标达成：审查全部问题（H1-H3b/M1-M5/Low）已闭环或明确保留记录；Review 结论的两处第一性原理修正（消息落库实有 + patch 语义）已固化入行为与契约测试。

### 1k. 只读审查问题修复（第一性原理验证，2026-09-01）

Review 结论被实测修正（第一性原理）：user 消息实已落库（handleChatStream L592-596）、ask/agent/mock 成功路径均落库 assistant —— 唯一真实缺口是 **error（非中止）路径丢弃已流出的 partial**。

已修复：

- H1：chat-loop handleStreamFailure 错误分支先抢救 partial 落库（与 aborted 对称、末条去重），E2E 新增 scene 1b 断言（session.get 含 user+assistant）

- H2：App.deleteSession 以 refresh 后真实列表重置 activeId（顺带修陈旧闭包：refresh 改为返回 mapped）

- H3a：ipc-guard 新增 clearSessionGuardState(sessionId)，ipc.ts session-delete 联动调用；强测用「终态后 chunk 被拒 + 清除后恢复播种」

- M4：chat-stream-state done/error 终态清理该会话幂等 key（防无界）；集合内部不可黑盒断言，用新用例「跨会话同名 key 隔离」替代无效测试

- M1：MessageBubble CodeBlock children 数组 join("") 防御（避免 String() 逗号注入）

- M2：main/index.ts uncaughtException 日志后 app.exit(1)（不再带病运行；unhandledRejection 保留可恢复语义）

验证实测：typecheck 0；vitest **461/461**（+2 新用例）；build 绿（renderer 1112KB）；真实会话 E2E 3 断言（ask 渲染 + 消息落库 + agent 审批）全过、零 console 错误。

未修（按审查记录保持）：H3b/M3 守卫豁免面契约测试扩展（规划）、M5 patch 污染流式文本（产品语义确认）、Low 项记录。

### 1j. 前端生产级迭代第 3 轮（ADHD 发散 × ponytail 收敛，2026-09-01）

发散候选 6 项 → 采纳 3 项 / 砍 3 项（流式逐块 Markdown 解析（性能）、busy 禁输（允许插话是特性）、消息合并（会话消息由主进程组装））。已落地：

- 模式切换 pill（ChatView）：header 循环切换 ask→suggest→auto→fullAuto，读/写 window\.dave.store（持久化），i18n 文案

- assistant 消息 Markdown 渲染（MessageBubble）：react-markdown + hljs 按需注册 10 语言（ts/js/json/bash/sh/python/css/xml/html/md/diff）；流式/aborted/tool 保持纯文本（只解析一次）；hljs core+语言注册把 renderer bundle 从 2511KB 压回 1112KB（-56%）

- 侧栏空态引导（Sidebar）：无会话时显示"暂无会话 + 点击新对话"（区分搜索无结果）

i18n：新增 common.noSessions/createFirst（zh/en 同步，unit 校验通过）。

验证实测：typecheck 0；vitest 459/459；build 绿（renderer 1112KB JS + 30.4KB CSS）；真实会话 E2E 两场景 + 无 console 错误全过。

### 1i. 前端生产级微迭代（ADHD 发散 × ponytail 收敛，2026-09-01）

发散候选 7 项 → 采纳 4 项 / 砍 3 项（空态动效、键盘导航低边际；侧栏实时刷新已由命名触发覆盖）。已落地：

- 会话自动命名（ChatView）：首条消息截 36 字 → window\.dave.session.updateTitle + App 刷新侧栏（titledRef 防覆盖）

- Esc 中止流式（ChatView）：busy（streaming/tool/approval）时全局 Escape 监听 → bridge.abort

- 消息复制（MessageBubble）：assistant/tool 气泡 hover 复制按钮（Copy/Check 态切换，clipboard API）

- 删除二次确认（Sidebar）：删除按钮点击进入"✓ 确认"态，再点才执行；选中会话或失焦重置

- i18n：新增 common.copy/copied/confirmDelete（zh/en 同步）

验证实测：typecheck 0；vitest 459/459；build 绿（renderer 696KB）；真实会话 E2E 两场景 + 无 console 错误全过（含新会话自动命名路径）。

### 1h. 前端企业生产级改进（ADHD 发散 × ponytail 收敛，2026-09-01）

发散候选 12 项 → 采纳 5 项 / 砍 6 项（虚拟列表、主题切换、消息重发、自动命名、骨架屏、错误上报链路——前两者 YAGNI/无需求，其余等待上游协议）。

已落地（全部通过验证）：

- ErrorBoundary（renderer/components/ErrorBoundary.tsx）：root 级错误隔离 + 重试；role=alert 降级面板

- App 数据层（App.tsx）：window\.dave.session.list/create/delete 真实会话驱动侧栏（今天/昨天/更早分组、空态），activeId 永不悬挂

- ChatView（components/ChatView\.tsx）：挂载时 session.get 补拉真实历史、空态引导（i18n 文案）、状态区 role=status + aria-live 播报、错误 role=alert

- Sidebar（components/Sidebar.tsx）：受控 props（sessions/onSelect/onNew/onDelete），移除 DEMO 数据

- i18n：空态复用 chat.placeholder（zh/en 同步完好）

验证实测：typecheck 0；vitest 459/459；build 绿（renderer 692.8KB）；真实会话 E2E 两场景 + 无 console 错误全过（新流程含"创建首个真实会话"步骤）。

### 1g. 真实会话端到端验证完成（2026-09-01）

- 新增 tests/chat-stream.e2e.mjs（npm run chat:e2e —— build 后驱动真实 Electron + mock provider）：

  - 场景 1 ask：UI 输入 → window\.dave.chat.stream → 主进程 mock → pushWithGuard(start/chunk/done) → preload → bridge → store → ChatView 流式渲染 → done 落常驻消息，断言"这是 mock 回复：你好 Dave"

  - 场景 2 agent：store.mode=auto → 输入 → ApprovalCard 出现 → 点允许（IPC approve + 本地 approval\_result）→ 工具轮后最终文本含"（mock 工具轮已完成，模式=auto）"

  - 附加断言：全程无 console 错误

- 修复两个真实链路缺陷：

  1. 状态机缺 tool\_pending→approval 转移（mock/真实 agent 工具轮在渲染端被拒、审批卡永不出现）→ chat-stream-state.ts 补该转移 + 单测 19
  2. ChatView 流结束后 finalContent 需落为常驻消息（否则 done 后文本消失）→ done 落 history（useRef 去重）

- 验证实测：chat-stream-state 20/20、全量 459/459、build 绿、真实会话 E2E 两场景 + 无 console 错误全过（exit 0）

- 遗留：electron-smoke.mjs 面向旧 renderer UI（welcome/cmdk/.msg-row 等）已与新 ChatView 不匹配，重写为 chat-stream.e2e.mjs（真实会话门禁）。

**当前验证全貌**：unit 459（npm test）、前端原型 E2E 18/18（preview:e2e）、真实会话 E2E 2 场景（chat:e2e）、构建绿（build）、typecheck 双零错。

### 1f. renderer 源码树补全 + ChatView/ApprovalCard 集成（2026-09-01）

补齐 src/renderer 完整结构（对齐 HANDOFF 旧惯例：components/i18n/styles 在 renderer 根级，无 src/ 嵌套）：

- 入口：src/renderer/index.html + main.tsx + App.tsx（活动栏 40/侧栏 260/ChatView 布局）

- i18n：src/renderer/i18n/index.ts（zh-CN/en key 树一致，命名导出 resources）——**修复 tests/unit.test.ts 的 TS2307 与运行失败**

- 样式：src/renderer/styles/globals.css（Tailwind v4 入口 + 基础 token）

- 组件：ChatView（渲染端消费接线集成点：store+bridge+状态驱动 UI）、ApprovalCard（审批卡：允许/拒绝/mutates/shell 风险标记）、MessageBubble、MessageInput、ActivityBar、Sidebar

- 桥接链路：preload onStart → use-chat-stream-bridge(7 通道订阅+sessionId 过滤+buildEventFromChannel 复用) → createChatStreamStore → useChatStreamStore

- 类型：src/renderer/env.d.ts 全局声明 window\.dave（DaveApi 来自 preload）

**验证（三层全绿）**：tsc 根/node 双零错；vitest **458 passed / 0 failed**（unit.test.ts 全部恢复，此前 286+1fail）；npm run build 全链路成功（main 367KB/preload 10.5KB/renderer 686KB JS+26.7KB CSS）。

**剩余**：真实会话 E2E —— 需完整 renderer 源码（settings/skills 等其它视图）与 electron 运行验证（smoke 断言渲染端 store 文本对齐），属后续增量。

### 1e. 前端原型 + 开源选型 + E2E 门禁（2026-09-01）

- 前端原型：frontend-preview/index.html（暖琥珀 light-first ChatView：动作轨道、待批托盘、锚点光标、撤销语义、空态、模式切换、会话搜索，可交互回放）。预览 node frontend-preview/server.mjs（:5177）。

- 修复：待确认托盘/补丁弹层曾被输入区遮挡不可点 → z-index 40/50 + placeOverlay() 跟随 composer 实际高度动态定位（输入增高/窗口 resize 均重算）。

- 开源优先选型（GitHub/npm 生态调研，2026）：组件层采用 shadcn/ui（MIT · Tailwind4 · 代码进库零 runtime 依赖）；复用既有 @tanstack/react-virtual、react-markdown、lucide、i18next、zustand、diff。拒绝 Mantine（主题双轨冲突）、自研全组件。详见 RENDERER\_CONSUME\_PLAN.md §8。

- E2E 验证门禁：tests/frontend-preview\.e2e.mjs（playwright chromium，复用已有依赖；首次需 npx playwright install chromium）。18 个 named-risk 检查（R1-R12，含遮挡回归 R4b、控制台错误 R12）。实测 18/18 通过。命令 npm run preview:e2e。完整 check matrix 与失败响应见 RENDERER\_CONSUME\_PLAN.md §9。

- 成功/验证标准（四层）：unit 458 全绿（npm test，unit.test.ts 已恢复）、E2E 18（preview:e2e）、构建 npm run build 绿、electron smoke（会话全链路后续增量）。

上轮 ADHD 收敛的未落地项已全部处理，277 单测全绿（唯一失败为预存 unit.test.ts 缺 renderer/i18n）：

**Wave-A — IPC 推送时序前置断言（已落地）**

- `src/main/security/ipc-guard.ts`：registerPushChannel options 新增 mapEvent/sessionIdOf；pushWithGuard 在 schema 校验后按会话状态机做转移校验，非法转移（idle 直 done、done 后 chunk、重复 done）抛错不 send 并累计违规；新增 getPushViolationStats()；resetPushRegistry 清空会话状态机缓存与违规统计

- `src/main/security/push-channels.ts`：删除 description 字段（审计发现零消费）；6 通道全部配置 rateLimit；新增 chat-stream-start 通道

- 新测试 tests/ipc-push-sequence.test.ts（7 例）

**Wave-B — 幂等 key 会话维度化（已落地）**

- `src/shared/chat-stream-state.ts`：幂等去重改为 `${sessionId}:${idempotentKey}` 命名空间，start 清该会话 keys、reset 全局清空；公开签名不变

- 新增 5 测试（跨会话不串扰 ×2、重连重放、reset 重放、store 跨会话）

**Wave-C — 端到端事件契约闭环（已落地）**

- 新 `src/shared/event-contract.ts`：runContractTrail 回放事件序列产出 final+violations；renderTranscript 还原渲染输入文本

- 新 tests/event-contract.test.ts（7 例）：完整序列拼出"你好，世界"、孤儿 chunk 计入违规、done 后 chunk 违规且 final 保持、patch 覆盖、reset 重来

**集成修复（关键决策）**

- 生产 chat-loop 首事件原为 chunk 无 start → 时序守卫会拦杀全部流。修复：push-channels 新增 chat-stream-start 通道；chat-loop.ts 真实路径 + mock 路径在流开始前 push start（断线重连再 start 合法）

- error/tools/approval/patch 四通道**免除时序守卫**（保留 schema+限流）：多轮 agent 工具循环产生连续 tools/approval/patch，状态机模型只支持 streaming 或 tool\_pending 互斥；免除后状态机停留 streaming，后续 chunk 仍合法。渲染端 store 的 approval\_result 由本地用户交互产生，不依赖主进程推送

- 守卫最终范围：**start/chunk/done**（核心防御：孤儿/双 done/done 后数据全被拦）

- 测试 tests/ipc-push-sequence.test.ts 用例 4 适配免除语义

**验证**：全量 vitest 277 通过；npx tsc --noEmit（renderer+shared+preload）零错误；tsconfig.node.json 唯一报错为预存 unit.test.ts。

**落地判定（事实核查，grep + 测试证据）：**

| 环节                    | 判定     | 证据                                                    |
| --------------------- | ------ | ----------------------------------------------------- |
| pushWithGuard 替换 18 处 | 已落地    | grep 0 残留 `event.sender.send("chat-stream-` + 17 测试全绿 |
| 6 通道 schema 启动注册      | 已落地    | index.ts L144-145 接线 + 8 测试全绿                         |
| 状态机/store/hook 模块     | 已建成未接入 | 26 测试全绿，生产 0 引用                                       |
| 跨域一致性门禁               | 部分落地   | 13 测试仅覆盖主进程 SessionRuntime 内部                         |
| 时序合法性（done 后 chunk）   | 未落地    | pushWithGuard 只校验 payload 形状                          |
| 推送通道限流                | 未落地    | 6 通道 rateLimit 全部缺省                                   |
| 渲染端消费接线               | 未落地    | renderer 源码缺失，hook 无消费方                               |

**ADHD 三个最高杠杆建议（未实施，待续）：**

1. 时序合法性前置断言 — 状态机焊进 pushWithGuard，先单通道试波及非误杀
2. 端到端事件契约闭环（collector）— 不依赖 renderer 证明链路，golden snapshot 锁定
3. 真实流量影子回放 — pushWithGuard 旁路喂状态机，只写 trace 不上屏

**Ponytail 审计要点（不实施，供决策）：**

- 两套 sender 校验并存（ipc.ts validateSender 30+ 处 vs createIpcSecurity）→ 历史遗留过渡态，触犯"不留兼容层"

- store/hook 零生产消费者 → 交付物保留，renderer 补齐前**不加层**

- PushChannelOptions.description 6 处赋值零消费 → 可删

- 幂等 key 为 chat-stream-state 模块级全局 Set → 多会话并发会跨会话撞 key 误跳（生产债，非 ponytail 范畴）

- tests 中无 `ponytail:` 注释标记，债务台账干净

**已完成全部 TDD 重建步骤：**

- Step 1（ipc-guard 推送通道注册）✓

- Step 2（chat-stream-state 状态机）✓

- Step 3-4（chat-stream-store + useChatStreamStore React hook）✓

- Step 5（chat-loop.ts 18 处 event.sender.send() → pushWithGuard()）✓

- Step 6（跨域一致性门禁）✓

- 新增测试共 64 个全绿（8+15+11+17+13）

**剩余接线（P2，待 renderer 源码就绪）：**

- ChatView 组件消费 `useChatStreamStore`，IPC 推送事件（chunk/done/error/approval/patch/tools）→ `store.dispatch()`

**前置依赖：** ChatView 接线依赖 renderer 源码目录存在（当前工作区为 renderer 不完整副本）。
