...
> **更新**: 2026-09-04（Asia/Shanghai）— **v0.4 全链路就绪：内部项收口 + 发布候选 + 版本 0.4.0**。第 19-20 轮：文档全量对齐（OVERVIEW/archcore/RESIDUAL_RISKS/CHANGELOG/发布说明）+ 环境后台格式化器根因处置（entry-points/CHANGELOG 已 skip-worktree + .prettierignore）+\
...
| entry-points.doc.md / CHANGELOG.md / tests/V0_4_GATES.md 被环境后台格式化器持续改写 | 已实证：`git restore` 后约 1 秒，无任何命令/钩子参与，文件被后台 watcher 重写为固定 prettier 变体输出（frontmatter `---`→`***` + `\.` 转义 + 空行；CHANGELOG 标题 `[0.4.0]` 被转义成 `\[0.4.0]`；V0_4_GATES 为表格列宽重排），与 commit 钩子无关（`--no-verify` 提交后同样复发） | **已设** **`git update-index --skip-worktree`** **：`.archcore/architecture/entry-points.doc.md`、`CHANGELOG.md`、`tests/V0_4_GATES.md`**（git 永久忽略本地漂移，HEAD/远端保持干净）；三者均已入 `.prettierignore`；不要 `git add` 这些文件；真要改：先 `--no-skip-worktree`，改完提交后重设 |
...
### 推荐工作顺序（2026-09-04 更新——此前 8 步全部完成，不再适用）

1. 先 `git status` + `git log -3 --oneline` + 读本文件，确认处于 master `b5b2949` 及以上、工作区干净
2. 运行 `node tests/verify-full.mjs`（7 步：ipc-consistency → sender-coverage → build → unit 489 → chat:e2e 6 → preview 18 → uat 6），CI 亦可作权威证据
3. 剩余外部解锁项（按用户资源就绪顺序）：SIGNING（证书 → 重推 tag）→ UAT-E2E-REAL（`DAVE_REAL_API_KEY` → `npm run chat:e2e:real`）
4. 新功能方向：按 staff-engineer + TDD（红-绿-重构）实施，新 IPC 能力必须走 security.handle 契约 + 推送到 registerPushChannel
5. 文档同步习惯：改动落地后同步 HANDOFF/README/OVERVIEW/archcore/RESIDUAL\_RISKS/CHANGELOG/发布说明；doc 提交后跑 `npx prettier --write`（注意 env 变体双稳定点文件已入 .prettierignore）；`.archcore` 与 `CHANGELOG.md`/`entry-points.doc.md` 已 skip-worktree，勿 `git add`
...
【状态】git 已初始化（master @ b5b2949，已推送）· origin 已关联（langkeshuo-ai/dave-desktop）· CI 连续全绿 · v0.4.0 已公开发布 · light-first · Activity Bar 布局 · 无自动 onboarding
【安装目录】C:\Users\C\AppData\Local\Programs\dave-desktop
【最新打包】dist-v9/win-unpacked/（v0.4.0 全功能；本地安装目录已于 2026-09-04 更新为 dist-v9）
【必读】HANDOFF.md（2026-09-04）+ tests/V0_4_GATES.md
【先跑】node tests/verify-full.mjs（7 步：ipc-consistency → sender-coverage → build → unit 489 → chat:e2e 6 场景 → preview:e2e 18/18 → uat 6，CI 持续全绿）
