...
| CI               | P2   | workflow 已入库；远端 runner 首次绿灯与 package smoke 仍待观察;**本地无 git remote(2026-07-31 确认);GitHub MCP 创建仓库尝试失败(fetch failed,服务不可达),需配置 remote 后 push 才能触发** | PR 上 `npm ci` + verify + electron smoke 稳定；可选 package:win smoke  |
