...
> 状态：**Phase 1 进行中**（安全架构模块已迁移，待集成到 ipc.ts）
> 创建日期：2026-08-26
> 最后更新：2026-08-26
...
- ✅ **Phase 0**：`docs/merge/` 目录创建，合并计划文档就位
- ✅ **Phase 1.1**：`ipc-guard.ts` + `rpc-hub.ts` + 综合测试（50+ 用例）已创建
- ✅ **Phase 1.2**：`tool-capability.ts`（HMAC-SHA256 一次性能力令牌）已创建
- ✅ **安全模块统一导出**：`src/main/security/index.ts`

**待完成**：将安全模块集成到 `ipc.ts`（替换现有内联校验）、browser-policy 迁移、Phase 1 全量验证。
...
├── hashToolRequest：SHA256 工具请求哈希
├── summarizeToolInput：敏感键过滤摘要
└── createToolCapabilityAuthority：HMAC-SHA256 一次性能力令牌

security.test.ts ✅（50+ 测试用例，覆盖三模块）
```
...
- ✅ **新增**：`src/main/security/` 独立安全模块（ipc-guard + tool-capability + rpc-hub）
- ⏳ 待集成：安全模块接入 `ipc.ts`，替换现有内联校验
- ❌ 缺少 BrowserHub 安全策略（Phase 1.3）
...
- [x] 用 TypeScript 重写 `ipc-security.mjs` → `src/main/security/ipc-guard.ts`
  - 来源校验：`assertTrustedSender(event)` 函数
  - payload 校验：`inspectValue(value)` 函数（深度/键数/字符串长度/危险键/循环检测/纯对象）
  - 危险键列表：`__proto__`, `constructor`, `prototype`
  - 路径信任根：`assertAllowedShellPath(targetPath, roots)`
  - zod schema 集合：`channelSchemas`
- [x] 新增 `src/main/security/rpc-hub.ts`（JSON-RPC 2.0 完整 hub + 中间件 + 批量请求）
- [x] 新增 `src/main/security/index.ts`（统一导出）
- [x] 创建综合测试 `src/main/security/security.test.ts`（50+ 用例，覆盖 ipc-guard/tool-capability/rpc-hub）
...
- [x] 用 TypeScript 重写 `tool-capability.mjs` → `src/main/security/tool-capability.ts`
  - `hashToolRequest`：SHA256 工具请求哈希（tool + workspace + input）
  - `summarizeToolInput`：敏感键过滤摘要
  - `createToolCapabilityAuthority`：HMAC-SHA256 一次性能力令牌（60秒TTL，timingSafeEqual）
...
| Phase 1 | 安全架构迁移 | 2-3天 | P0 | 🔄 进行中（模块已创建，待集成） |
| Phase 2 | 测试体系迁移 | 1-2天 | P1 | ⏳ 待开始 |
| Phase 3 | 功能模块迁移 | 3-5天 | P1 | ⏳ 待开始 |
...
| Phase 1 | 🔄 进行中 | 2026-08-26 | ipc-guard/rpc-hub/tool-capability + 测试已创建，待集成到 ipc.ts |
| Phase 2 | ⏳ 待开始 | | |
| Phase 3 | ⏳ 待开始 | | |
| Phase 4 | ⏳ 待开始 | | 可选 |
| Phase 5 | ⏳ 待开始 | | |
