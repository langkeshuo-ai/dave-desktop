...
> **最新实测（2026-09-03，v0.4.0 终态）**：冷启动 464ms（预算 3s）、2000 条消息真实滚动 avg 144fps / P95 7ms / P99 7.1ms / 慢帧 0。权威数字以 `HANDOFF.md`（2026-09-04）与 `INTEGRATED_OVERVIEW.md` §1 为准；本报告为 0.2.0 阶段（2026-07-27/31）历史快照。
...
| 2000 消息滚动帧率目标 | 未测试       | >50fps       | **✅ 60fps（2026-07-31 真机实测,P95 16.8ms）** |
| 流式 Markdown 重解析  | 每 chunk     | 节流         | **✅ 120ms 节流 + memo**                       |
| 敏感 IPC 限流         | 无           | 有           | **✅ store/chat/patch**                        |
...
**实测结果（2026-07-31，真机 Windows，`tests/electron-fps.mjs` 自动采集）**:

| 场景     | 消息数 | 实测 avg FPS | P95 帧间隔 | P99 帧间隔 | >33.3ms 慢帧 | 结论                                |
| -------- | ------ | ------------ | ---------- | ---------- | ------------ | ----------------------------------- |
| 极限压测 | 2000   | **60**       | **16.8ms** | **16.8ms** | **0**        | ✅ 达标(>50fps、P95<30ms、P99<50ms) |

采集方式:注入 2000 条混合消息(70% 文本 + 15% 代码块 + 15% diff patch),
reload 渲染后滚动列表,rAF 帧间隔统计滚动期间帧率。

**冷启动实测（2026-07-31，`tests/electron-coldstart.mjs`，读取主进程 `first_window_shown` 遥测点）**:

| 指标                       | 实测       | 预算(COLD_WINDOW_BUDGET_MS) | 0.2.0 目标 | 结论                                                                              |
| -------------------------- | ---------- | --------------------------- | ---------- | --------------------------------------------------------------------------------- |
| 主进程拉起 → ready-to-show | **1726ms** | 3000ms(within ✅)           | <1500ms    | 预算内达标;距激进目标差 226ms(createWindow 前置重排已见效,剩余大头为渲染进程加载) |
