...
- [ ] **Sentry 集成** — 捕获未处理异常 + React Error Boundary(本地优先:产品约束不上报,倾向本地诊断)
- [ ] **匿名遥测** — 可选开启（Settings 明确告知），收集崩溃率 / 使用频率(同上,本地优先)
- [x] **本地诊断工具** — 一键导出"诊断报告"(2026-07-31):`src/main/diagnostics.ts` 打包系统信息 + 会话元数据 + 应用日志 + 结构化事件日志为单个文本文件,Settings「关于」按钮触发
...
## 十、与参考 CLI/Agent 项目的差距矩阵(2026-07-31 评估)

对照 atomcode / claude code / codex / hermes 等项目的能力清单:

| 能力域                                 | 参考项目 | dave-desktop                         | 状态           |
| -------------------------------------- | -------- | ------------------------------------ | -------------- |
| Agent 工具循环(shell/写文件/patch/AST) | ✅       | ✅                                   | 无差距         |
| 多 provider 流式                       | ✅       | ✅ 4 provider + 自定义               | 无差距         |
| 审批/安全纵深                          | ✅       | ✅ 4 模式 + IPC 白名单/限流/sanitize | 无差距         |
| 会话管理/导出/搜索                     | ✅       | ✅                                   | 无差距         |
| 性能(bundle/FPS/冷启动)                | ✅       | ✅ 实测达标                          | 无差距         |
| MCP 工具扩展                           | ✅       | ✅ 基础版(2026-07-31)                | 已对齐         |
| 结构化日志/可观测性                    | ✅       | ✅ JSON Lines + 查看器               | 已对齐         |
| 诊断导出                               | ✅       | ✅ 一键打包                          | 已对齐         |
| skills / 工具市场                      | ✅       | ❌                                   | 0.3.0 记录     |
| 多端/手机接入                          | 部分     | ❌                                   | 桌面应用非必要 |

**语言重构评估(结论:不重构)**:现有栈(Electron 42 + React 19 + TS 5.8)现代活跃;
153+ 单测与全部功能对等重写回归风险极高;Tauri 对 Node 工具链与 MCP SDK(Node 客户端)
适配成本高;无用户可见收益。符合开源优先原则的"确凿理由"豁免条款。
