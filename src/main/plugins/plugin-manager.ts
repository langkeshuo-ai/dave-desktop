...
export interface PluginInfo {
  ...
  error?: string
  loadedAt?: number
  /** 连续执行失败计数（触发自动禁用的退避额度） */
  fails?: number
  /** 连续失败达到阈值后自动禁用，后续加载被拒绝（429 语义） */
  disabled?: boolean
  disabledAt?: number
}

/** 连续失败阈值：达到后自动禁用插件，防止市场扩展崩溃循环 */
export const PLUGIN_FAIL_THRESHOLD = 3
...
export class PluginManager extends EventEmitter {
   ...
   * 已禁用的插件直接拒绝（429 语义）；入口缺失计一次失败，连续达阈值自动禁用；
   * 加载成功清零退避计数。
  ...
  loadPlugin(name: string): PluginInfo {
    ...
    if (plugin.disabled) {
      ...
      plugin.error =
        plugin.error ||
        `plugin '${name}' disabled after ${PLUGIN_FAIL_THRESHOLD} consecutive failures`
      return plugin
    }
  ...
  /**
   * 记录一次执行失败：连续失败达 PLUGIN_FAIL_THRESHOLD 次自动禁用插件
   * 并触发 plugin:disabled 事件。未知插件返回 undefined。
   */
  reportFailure(name: string): PluginInfo | undefined {
    const plugin = this.plugins.get(name)
    if (!plugin) return undefined
    plugin.fails = (plugin.fails || 0) + 1
    if (plugin.fails >= PLUGIN_FAIL_THRESHOLD && !plugin.disabled) {
      plugin.disabled = true
      plugin.disabledAt = Date.now()
      plugin.status = "error"
      plugin.error = `plugin '${name}' disabled after ${plugin.fails} consecutive failures`
  ...
  /**
   * 记录一次成功（如升级成功）并清零退避计数、解除禁用。
   * 未知插件返回 undefined。
   */
  reportSuccess(name: string): PluginInfo | undefined {
    const plugin = this.plugins.get(name)
    if (!plugin) return undefined
    plugin.fails = 0
    plugin.disabled = false
  ...
  hasPermission(pluginName: string, permission: string): boolean {
    ...
    if (!plugin) return false
    return (
      plugin.manifest.permissions.includes(permission) || plugin.manifest.permissions.includes("*")
    )
  }
  ...
  getStatus(): Record<string, unknown> {
    return {
      ...
      byStatus: this.listPlugins().reduce(
        (acc, p) => {
