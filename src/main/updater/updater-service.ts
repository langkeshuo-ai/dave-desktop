...
function validateFeedUrl(value: string, options: { allowLocalhost?: boolean } = {}): string {
  ...
  if (url.protocol !== "https:" && !(options.allowLocalhost && isLocalhost && url.protocol === "http:")) {
...
async function tryLoadElectronUpdater(): Promise<{ autoUpdater: { setFeedURL: (opts: { provider: string; url: string }) => void; autoDownload: boolean; autoInstallOnAppQuit: boolean; on: (event: string, handler: (...args: unknown[]) => void) => void; checkForUpdates: () => Promise<{ updateInfo?: unknown }>; downloadUpdate: () => Promise<void>; quitAndInstall: (isSilent: boolean, isForceRunAfter: boolean) => void } } | null> {
  try {
...
/** 获取当前更新状态 */
export async function getUpdateStatus(): Promise<UpdateState & { currentVersion: string; isPackaged: boolean; provider: string; notes: string }> {
  const state = readState()
  return {
    ...state,
 ...
 */
export async function checkForUpdates(options: { feedUrl?: string; download?: boolean } = {}): Promise<UpdateCheckResult> {
  const state = readState()
  ...
  const updateAvailable =
    Boolean(feedMeta?.version) && String(feedMeta.version) !== String(appVersion()) && !feedMeta?.error

  if (!appIsPackaged()) {
    const result: UpdateCheckResult = {
      status: "dev-mode",
      message: "Install/update apply is disabled in unpackaged/dev mode; feed metadata still checked",
      currentVersion: appVersion(),
      feedUrl: url || null,
      feedMeta,
      updateAvailable,
  ...
  const { autoUpdater } = loaded
  if (url) autoUpdater.setFeedURL({ provider: "generic", url })
  autoUpdater.autoDownload = options.download != null ? Boolean(options.download) : state.autoDownload !== false
  try {
    ...
    const result: UpdateCheckResult = {
      ...
      updateAvailable:
        updateAvailable ||
        Boolean((updateCheck?.updateInfo as { version?: string } | null)?.version && (updateCheck.updateInfo as { version: string }).version !== appVersion()),
    }
    writeState({ lastResult: result })
    return result
  } catch (err) {
...
/** 下载更新（仅打包模式） */
export async function downloadUpdate(): Promise<{ ok: boolean; message?: string; status?: string }> {
  if (!appIsPackaged()) {
