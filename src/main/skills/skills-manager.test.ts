...
describe("skills-manager", () => {
  ...
  beforeEach(() => {
    ...
    // Skills are stored in DAVE_HOME/skills/
    const skillsRoot = path.join(tmpSkillsDir, "skills")
...
`,
    ...
    // 创建第二个技能
    const skill2Dir = path.join(skillsRoot, "another-skill")
...
`,
    ...
    // 创建无 frontmatter 的技能
    const skill3Dir = path.join(skillsRoot, "no-frontmatter")
    fs.mkdirSync(skill3Dir, { recursive: true })
    fs.writeFileSync(path.join(skill3Dir, "SKILL.md"), "# No Frontmatter\n\nJust markdown body.")

    // quoted-skill for frontmatter parsing test
    const skill4Dir = path.join(skillsRoot, "quoted-skill")
    fs.mkdirSync(skill4Dir, { recursive: true })
    fs.writeFileSync(
      path.join(skill4Dir, "SKILL.md"),
      `---
name: quoted-skill
...
`,
  ...
  describe("readSkill path-safety", () => {
    it("rejects path traversal names (null)", () => {
      // 目录外构造同名 SKILL.md，验证穿越被拒绝而不是被读取
      const outside = path.join(tmpSkillsDir, "outside", "SKILL.md")
      fs.mkdirSync(path.dirname(outside), { recursive: true })
      fs.writeFileSync(outside, "SECRET", "utf8")

      expect(readSkill("../outside")).toBeNull()
      expect(readSkill("..")).toBeNull()
      expect(readSkill("../../etc")).toBeNull()
      expect(readSkill("a/b")).toBeNull()
      expect(readSkill("a\\b")).toBeNull()
      expect(readSkill(".hidden")).toBeNull()
      expect(readSkill("")).toBeNull()
      expect(readSkill("spaces in name")).toBeNull()
    })

    it("listSkills never exposes traversal entries", () => {
      const outside = path.join(tmpSkillsDir, "outside", "SKILL.md")
      fs.mkdirSync(path.dirname(outside), { recursive: true })
      fs.writeFileSync(outside, "SECRET", "utf8")

      const names = listSkills().map((s) => s.name)
      expect(names.includes("../outside")).toBe(false)
      expect(names.some((n) => n.includes("/"))).toBe(false)
      expect(names.some((n) => n.includes("\\"))).toBe(false)
    })

    it("getSkillReferencesPath safely rejects traversal names", () => {
      // 需要导入后调用；此处经 skillsSystemPrompt 走同一防线
      expect(skillsSystemPrompt(["../outside", "test-skill"])).toContain("Skill: test-skill")
      expect(skillsSystemPrompt(["../outside"])).not.toContain("SECRET")
    })
  })
