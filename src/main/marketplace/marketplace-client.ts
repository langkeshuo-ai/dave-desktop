...
/** 列出所有市场目录及其插件 */
export function listMarketplaces(): { known: KnownMarketplacesFile; catalogs: MarketplaceCatalog[] } {
  const known = readJson<KnownMarketplacesFile>(knownPath(), { version: 1, marketplaces: [] }) || { version: 1, marketplaces: [] }
...
export function listInstalledPlugins(): InstalledPluginsFile {
  return readJson<InstalledPluginsFile>(installedPath(), { version: 1, plugins: [] }) || { version: 1, plugins: [] }
}

function findCatalogPlugin(marketplaceId: string, pluginName: string): { cat: MarketplaceCatalog; plugin: CatalogPlugin | undefined; sourceDir: string | null } {
...
/** 卸载插件 */
export function uninstallPlugin(options: UninstallOptions): { ok: boolean; uninstalled: InstalledPlugin } {
 ...
 */
export async function updateMarketplace(marketplaceId = "dave-plugins-official"): Promise<{ ok: boolean; marketplaceId: string; pluginCount: number }> {
  ...
  if (!fs.existsSync(path.join(dir, ".git"))) {
    ...
    await run("git", ["clone", "--depth", "1", `https://github.com/${repo}.git`, dir], marketplacesRoot())
  } else {
    await run("git", ["pull", "--ff-only"], dir)
  }

  const cat = readJson<MarketplaceManifest>(path.join(dir, "marketplace.json")) ||
...
export function describePlugin(name: string, marketplace?: string): PluginDescription {
  ...
  const hit = installed.find((p) => p.name === name && (!marketplace || p.marketplace === marketplace))
  if (hit) {
    const pj =
  ...
  for (const cat of catalogs) {
    ...
    if (p) return { installed: false, marketplace: cat.id, ...p }
  }
