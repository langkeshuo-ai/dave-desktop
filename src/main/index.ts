import { app, BrowserWindow, Tray, Menu, globalShortcut, session, shell } from "electron"
...
import { registerChatStreamPushChannels } from "./security/push-channels"
...
// 修复 Windows 上不打开 DevTools 时白屏的问题:GPU 进程启动竞态导致首帧不渲染。
// 禁用硬件加速改用软件渲染,牺牲少量性能换取稳定首屏。
...
// BrowserPolicy 单例 — 外链/弹窗/导航的安全策略，在 createWindow 的 setWindowOpenHandler 中使用。
const browserPolicy = new BrowserPolicy()
...
if (!initialized) {
  ...
  app.whenReady().then(() => {
    ...
    // 注册流式聊天推送通道契约，确保 pushWithGuard 可用
  ...
  process.on("uncaughtException", (err) => {
    ...
    // 未捕获异常后进程状态不可信（定时器/单例锁/状态机可能损坏），立即退出而非带病运行
    app.exit(1)
  })
...
function createWindow(): BrowserWindow {
  ...
  win.webContents.setWindowOpenHandler(({ url }) => {
    // 使用 BrowserPolicy 校验外链：安全 https: 链接在系统浏览器打开，危险协议直接拒绝。
    const check = browserPolicy.checkPopup(url)
    if (check.allowed) {
      shell.openExternal(url).catch((err) => {
        log.warn("shell.openExternal failed:", err instanceof Error ? err.message : String(err))
      })
    } else {
      log.warn("blocked renderer window.open:", sanitizeLogField(url), "reason:", check.reason)
    }
    return { action: "deny" }
  })
