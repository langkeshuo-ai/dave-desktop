...
// Set DAVE_HOME to a unique temp directory for test isolation
const testHome = fs.mkdtempSync(path.join(os.tmpdir(), "dave-cp-home-"))
...
describe("checkpoints", () => {
  ...
  beforeEach(() => {
    ...
    // Clean checkpoints for this session to ensure test isolation
    const cpSessionDir = path.join(testHome, "client", "checkpoints", sessionId)
    fs.rmSync(cpSessionDir, { recursive: true, force: true })
  })
  ...
  describe("createCheckpoint", () => {
    ...
    it("generates unique IDs", async () => {
      const cp1 = await createCheckpoint(sessionId, {
        workspace: tmpWorkspace,
        files: ["hello.txt"],
      })
      const cp2 = await createCheckpoint(sessionId, {
        workspace: tmpWorkspace,
        files: ["hello.txt"],
      })
