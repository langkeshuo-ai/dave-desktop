...
export const SENSITIVE_KEYS = /(?:api[-_]?key|authorization|cookie|credential|password|secret|token)/i
...
export function summarizeToolInput(input: Record<string, unknown>): ToolInputSummary {
  ...
  const keys = allKeys.filter((key) => !SENSITIVE_KEYS.test(key)).sort().slice(0, 20)
 ...
 * ```
 */
export function createToolCapabilityAuthority(options: AuthorityOptions = {}): ToolCapabilityAuthority {
  const {
