...
import { isPathWithin, getTrustedRoots, homeDir } from "../utils/paths"
...
export class BrowserPolicy {
  ...
  checkExternalUrl(url: string): UrlCheckResult {
    ...
    if (!this.allowedProtocols.has(protocol)) {
      return { allowed: false, reason: `Protocol ${protocol} is not in allowlist`, protocol, hostname, port }
    }

    // 开发模式 http: 仅限 localhost
    if (protocol === "http:" && this.isDev) {
      if (hostname !== "localhost" && hostname !== "127.0.0.1") {
        return { allowed: false, reason: "HTTP only allowed for localhost in dev mode", protocol, hostname, port }
      }
      if (port && !this.devPorts.has(port)) {
        return { allowed: false, reason: `HTTP port ${port} not in dev allowlist`, protocol, hostname, port }
      }
    }

    // 生产模式 http: 一律拒绝（除非在白名单内，但默认白名单只有 https）
    if (protocol === "http:" && !this.isDev) {
      return { allowed: false, reason: "HTTP is not allowed in production", protocol, hostname, port }
    }
  ...
  checkLocalPath(targetPath: string): PathCheckResult {
    ...
    // Windows 网络/设备路径
    if (process.platform === "win32" && (/^\\\\/.test(targetPath) || /^\\\\[.?]\\/.test(targetPath))) {
      return { allowed: false, reason: "Network and device paths are not allowed", resolvedPath: null }
    }

    let resolved: string
    try {
      resolved = fs.existsSync(targetPath) ? fs.realpathSync.native(targetPath) : path.resolve(targetPath)
    } catch {
  ...
  checkNavigation(url: string, allowedHosts: string[] = []): UrlCheckResult {
    ...
    if (allowedHosts.length > 0) {
      const allowed = allowedHosts.some(
        (host) => result.hostname === host.toLowerCase() || result.hostname.endsWith(`.${host.toLowerCase()}`),
      )
      if (!allowed) {
        return { ...result, allowed: false, reason: `Host ${result.hostname} not in navigation allowlist` }
      }
    }
