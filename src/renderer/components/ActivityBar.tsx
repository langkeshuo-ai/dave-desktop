...
const btn = "grid h-[30px] w-[30px] place-items-center rounded-lg border border-transparent text-[var(--ink-3)] transition-colors hover:bg-[var(--amber-50)] hover:text-[var(--amber-600)]"
...
export function ActivityBar({ onOpenSettings }: { onOpenSettings?: () => void }) {
  ...
  return (
    <nav className="flex h-full w-10 flex-col items-center gap-1.5 border-r border-[var(--line)] bg-[var(--surface)] py-2.5">
      <button className={`${btn} ${active}`} title={t("common.workspace")} aria-label={t("common.workspace")}>
